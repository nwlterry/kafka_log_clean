...
var defaultSecrets = []secretPat{
	...
	{regexp.MustCompile(`(?i)((?:ssl\.(?:keystore|truststore|key)\.password|sasl\.jaas\.config|basic\.auth\.user\.info|ldap\.java\.naming\.security\.credentials)\s*[=:]\s*)([^\s,]+)`), `${1}x-redacted-secret-x`},
